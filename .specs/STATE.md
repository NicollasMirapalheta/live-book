...
- **In-progress**: **Fase 4 — Execute** (inline). `tasks.md` escrito. **Feito e pushado (commits `0524e76`→último):** T1 tokens, T2 componentes, T3 `MAX_BOOKS=50`+gate, T4 estante de lombadas, T5 balão+estante centralizada, T6 estados+magia (`Magic.tsx`), T7 mesa amadeirada, T8 folha (aquecido TODO o lilás do motor: guardas/vinco/tints/sombras/accents/ink-faint, só-pele `AD-032`, Fase 0 verde), T10 casca do leitor (pílula "← Biblioteca"), T11 cascas de rota migradas aos tokens (consertou `--app-accent` indefinido). **Feedback do autor atendido:** cor das lombadas (bug do cover-gradiente), volume novo nasce com 5 páginas em branco (`NewBookRoute`), álbum com mesa amadeirada (não mais branco). Roadmap: `/new` elaborado registrado (chip + Fase 5). 322 testes verdes; build/typecheck limpos; caracterização Fase 0 intacta.
- **Next step**: **restam T9/T13/T14/T15** — dependem de **verificação no navegador/UAT** (não dá screenshot headless neste ambiente): T9 polimento do `album` (precisa de fotos reais), T13 responsivo 320→desktop, T14 contraste AA, T15 não-regressão de FPS (e2e Playwright). Autor deve olhar o preview (home + leitor amadeirado + criar/importar) e dar UAT. Depois, **Fase 5 do roadmap (editor)**; chip `task_d8a1e368` para o `/new` elaborado.
